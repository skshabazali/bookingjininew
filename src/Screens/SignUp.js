    ...
    SignUp=(email,password)=>{
        ...
        .then(async fcmToken => {
          if (fcmToken) {
            ...
            let urls=url+port+'/create-hotel/?'
               
                console.log("fcmToken is ",this.state.fcmTokens);
                axios.get(urls+`name=${this.state.name}&email=${this.state.email}&password=${this.state.password}&device_token=${fcmToken}&auth_key=${this.state.userId}&phone=${this.state.phonenumber}`
    )
    .then((response)=>{
